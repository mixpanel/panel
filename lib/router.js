...
export default class Router {
  constructor(app, options = {}) {
    ...
    if (isPathRouteConfig(routeDefs)) {
  ...
  invokeHashRouteHandler(fragment, compiledHashRoutes, stateUpdate = {}, pathParams = []) {
    ...
    for (const route of compiledHashRoutes) {
      ...
      if (matches) {
        ...
        const routeStateUpdate = routeHandler.call(this.app, stateUpdate, ...pathParams, ...params);
        if (routeStateUpdate) {
  ...
  replacePath({path = null, historyMethod = null, fragment = null} = {}) {
    historyMethod = historyMethod || this.historyMethod;

    const basePathMatch = this.window.location.pathname.match(this.basePathExpr);
    const fullPath = `${basePathMatch[0]}${path}`.replace(/\/$/, ``); // remove trailing slash

    let url = ``;
    if (path && !decodedFragmentsEqual(this.window.location.pathname, fullPath)) {
      url += fullPath;
    }
    if (fragment && !decodedFragmentsEqual(stripHash(this.window.location.hash), stripHash(fragment))) {
