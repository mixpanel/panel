...
const SHOULD_UPDATE_DEFAULT = (pre, cur) => pre !== cur;
...
class Component extends WebComponent {
  ...
  _syncParams() {
    // maintain local validated map where all schema keys are defined
    this._paramsSchemas = {};
    const paramsSchemas = this.getConfig(`params`);
    ...
    for (const param of Object.keys(paramsSchemas)) {
      // convert type shorthand to object
      let paramSchema = paramsSchemas[param];
      if (PARAM_TYPE_DEFAULTS.has(paramSchema)) {
      ...
      if (!PARAM_TYPE_DEFAULTS.has(type)) {
        ...
        throw new Error(
          `Invalid type: ${typeString} for attr: ${param} in paramSchema. ` +
      ...
      if (paramSchema.default && paramSchema.required) {
        throw new Error(`${this}: param '${param}' cannot have both required and default`);
      }

      if ([Object, Array, Map, Set].includes(type) && !paramSchema.shouldUpdate) {
        throw new Error(
      ...
      const paramSchemaObj = {
        type,
        default: paramSchema.default ? paramSchema.default : PARAM_TYPE_DEFAULTS.get(type),
        required: paramSchema.required ? paramSchema.required : false,
        // by default use === for comparision
        shouldUpdate: paramSchema.shouldUpdate || SHOULD_UPDATE_DEFAULT,
      };

      // set default value for the params
      this.params.$params[param] = paramSchemaObj.default;

      this._paramsSchemas[param] = paramSchemaObj;
    }

    const paramDescription = Object.keys(paramsSchemas)
      .map((param) => [
        param,
        {
          get() {
  ...
  setParams(params) {
    ...
    for (const param in params) {
      ...
      if (this._paramsSchemas[param].shouldUpdate(oldParam, newParam)) {
        if (this._paramsSchemas[param].required && (newParam === undefined || newParam === null)) {
          throw new Error(`${param} is required, ${newParam} passed in`);
        }
        toUpdate[param] = newParam;
      }
    }
    if (this.initialized) {
      this.runHook(`preUpdate`, updateOptions, this.state, Object.assign({}, this.params.$params, toUpdate));
    }
    Object.assign(this.params.$params, toUpdate);
    if (this.initialized) {
      this.domPatcher.update(this.state);
      this.runHook(`postUpdate`, updateOptions, this.state, this.params);
    }
  }
