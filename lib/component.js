...
class Component extends WebComponent {
  ...
  connectedCallback() {
    ...
    Array.from(this.attributes).forEach(attr => this._updateAttrs(attr.name));
  ...
  static get observedAttributes() {
    return Object.keys(Component.attrsSchema).concat(Object.keys(this.attrsSchema));
  }

  attributeChangedCallback(attr, oldVal, newVal) {
    this._updateAttrs(attr);
  ...
  _updateAttrs(attr) {
    ...
    if (attrsSchema.hasOwnProperty(attr)) {
      const propName = attrsSchema[attr].propName || this._toCamelCase(attr);
      const propType = attrsSchema[attr].type || `string`;
      let propValue = null;
  ...
  _toCamelCase(attr) {
    return attr
      .split(`-`)
      .map((word, i) => i === 0 ? word: `${word.charAt(0).toUpperCase()}${word.substr(1)}`).join(``);
  }
