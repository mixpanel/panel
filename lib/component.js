...
import shallowCompare from './component-utils/shallowCompare';
...
class Component extends WebComponent {
  ...
  setParams(params) {
    ...
    if (!this.shouldComponentUpdate(params, this.state)) {
      return;
    }
    const updateOptions = {
      cascade: false,
    ...
    if (this.initialized) {
      this.runHook(`preUpdate`, updateOptions, this.state, Object.assign({}, this.params.$params, params));
    }
    Object.assign(this.params.$params, params);
    if (this.initialized) {
  ...
  /**
   * same API as react's shouldComponentUpdate, by default use React.PureComponent's shallowCompare
   * If child class inherits this method, the child component should be respected
   */
