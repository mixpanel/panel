...
class Component extends WebComponent {
  ...
  update(stateUpdate = {}) {
    ...
    const rtn = this._updateStore(stateUpdateResult, {
      store: `state`,
  ...
  _syncParams() {
    ...
    const paramDescription = Object.fromEntries(
      Object.keys(paramTypes).map((param) => [
        param,
        {
          set(p) {
            if (!p) {
            ...
            if (self._paramTypes[param].required && (p === undefined || p === null)) {
              throw new Error(`${param} is required, ${p} passed in`);
            }
            if (self._paramTypes[param].shouldUpdate(this.$params[param], p)) {
              this.$params[param] = p;
              self.enqueueUpdate();
            }
          },
