...
class Component extends WebComponent {
  ...
  connectedCallback() {
    ...
    for (const attrsSchemaKey in this._attrsSchema) {
      if (!Object.prototype.hasOwnProperty(this._attrs, attrsSchemaKey) && this._attrsSchema[attrsSchemaKey].required) {
        throw new Error(`Missing required attribute '${attrsSchemaKey}' for ${this}`);
      }
    }
  ...
  _updateAttr(attr) {
    ...
    if (attrsSchema.hasOwnProperty(attr)) {
      ...
      if (!this.hasAttribute(attr)) {
        if (attrType === `boolean` && (attrSchema.default || attrsSchema.required)) {
          throw new Error(
        ...
        if (attrSchema.required) {
          // Early return because a required attribute has no known value
          // Booleans are a special exception, because even when explicitly set to false snabbdom will not have the attribute present in the DOM
          return;
        }
