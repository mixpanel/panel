...
class Component extends WebComponent {
  ...
  _updateAttr(attr) {
    ...
    if (attrsSchema.hasOwnProperty(attr)) {
      ...
      const attrType = (typeof attrSchema === `string`) ? attrSchema : attrSchema.type;
      const hasAttr = this.hasAttribute(attr);
      let attrValue = null;

      if (!hasAttr && (typeof attrSchema === `object`) && attrSchema.hasOwnProperty(`default`)) {
        attrValue = attrSchema.default;
      } else if (attrType === `string`) {
        attrValue = hasAttr ? this.getAttribute(attr) : ``;
      } else if (attrType === `boolean`) {
        attrValue = hasAttr ? this.isAttributeEnabled(attr) : false;
      } else if (attrType === `number`) {
        attrValue = hasAttr ? this.getNumberAttribute(attr) : 0;
      } else if (attrType === `json`) {
        attrValue = hasAttr ? this.getJSONAttribute(attr) : null;
      } else {
        throw new Error(`Unknown attrType: ${attrType} for attr: ${attr}`);
      }
