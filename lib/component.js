...
const PARAM_TYPE_DEFAULTS = new Set([Array, String, Boolean, Number, Object, Function, Map, Set]);
...
class Component extends WebComponent {
  ...
  _syncParams() {
    ...
    for (const param of Object.keys(paramSchemas)) {
      // convert type shorthand to object
      let paramSchema = paramSchemas[param];
      if (!paramSchema.type) {
      ...
      if (!PARAM_TYPE_DEFAULTS.has(type)) {
        ...
        throw new Error(
          `Invalid type: ${typeString} for param: ${param} in paramSchema. ` +
      ...
      if (defaultParams[param] && paramSchema.required) {
        throw new Error(`param '${param}' in ${this.constructor.name} cannot have both required and default`);
      }
      ...
      this._paramSchemas[param] = paramSchemaObj;
    }

    const paramDescription = Object.keys(paramSchemas)
      .map((param) => [
        param,
        {
          get() {
  ...
  setParams(params) {
    ...
    for (const param in params) {
      if (!this._paramSchemas[param]) {
        throw new Error(`extra param '${param}' on ${this.constructor.name} is not defined in schema`);
      }
      const newParam = params[param];
      const oldParam = this.params[param];
      if (this._paramSchemas[param].shouldUpdate(oldParam, newParam)) {
        if (this._paramSchemas[param].required && (newParam === undefined || newParam === null)) {
          throw new Error(`param '${param}' in ${this.constructor.name} is required, ${newParam} passed in`);
        }
        toUpdate[param] = newParam;
      }
    }
    // if nothing change, then we return in advance to avoid unecessary update
