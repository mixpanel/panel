...
class Component extends WebComponent {
  ...
  disconnectedCallback() {
    ...
    // if a child component is added via child() and has keys, snabbdom uses parentlEl.insertBefore
    ...
    // which causes a $panelParent not found exception.
    // we clean up parent references in an async manner so we can prevent that situation.
    if (this.$panelParent) {
      new Promise(res => res()).then(() => this._clearParentReferences());
    } else {
      this._clearParentReferences();
    }

    this.initialized = false;
  }

  _clearParentReferences() {
