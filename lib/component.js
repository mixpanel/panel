    ...
    this.postRenderCallback = (elapsedMs) => {
      if (elapsedMs > this.getConfig(`slowThreshold`)) {
        const shouldBroadcast =
          ...
          this.lastSlowRender.time - getNow() > 3000 || //SHOULD because last time was more than three seconds
          (elapsedMs - this.lastSlowRender.elapsedMs) / this.lastSlowRender.elapsedMs >= 0.05; //SHOULD because this time is more than 5% worst
        ...
        if (shouldBroadcast) {
          ...
          this.dispatchEvent(
            new CustomEvent(`slowRender`, {
              detail: {elapsedMs, component: this.toString()},
              bubbles: true,
              composed: true,
            }),
          );
