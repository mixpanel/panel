...
class Component extends WebComponent {
  ...
  connectedCallback() {
    ...
    if (Object.keys(this.getConfig(`routes`)).length) {
      const locationContext = this.hasContext(`location`) ? this.getContext(`location`) : window.location;
      const historyContext = this.hasContext(`history`) ? this.getContext(`history`) : window.history;
      ...
      this.router = new Router(this, {historyMethod: this.historyMethod, window: windowContext});
      this.navigate(locationContext.hash);
    }
  ...
  hasContext(contextName) {
    return this._contexts.has(contextName);
  }
}
