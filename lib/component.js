...
class Component extends WebComponent {
  ...
  static get metadata() {
    return {
      ...
      attrs: {
        'style-override': {
          type: `string`,
          description: `A raw css string that get's injected into the component's <style> if it uses shadowDom`,
        },
      },
      events: {},
    };
  ...
  static get observedAttributes() {
    return Object.keys(Component.metadata.attrs).concat(Object.keys(this.metadata.attrs));
  }
  ...
  attributeChangedCallback(attr, oldVal, newVal) {
    ...
    if (this.isPanelRoot && this.initialized) {
      const stateUpdate = this._stateUpdateFromFromAttr(attr, newVal);
  ...
  _stateFromAttributes() {
    ...
    for (let ai = 0; ai < this.attributes.length; ai++) {
      ...
      if (attrMatch) {
        ...
        Object.assign(state, this._stateUpdateFromFromAttr(attr));
      }
  ...
  _stateUpdateFromFromAttr(attr) {
    const attrsMetadata = this.constructor.metadata.attrs;
    if (attrsMetadata.hasOwnProperty(attr) && attrsMetadata[attr].hasOwnProperty(`propName`)) {
      const propName = attrsMetadata[attr].propName;
      const propType = attrsMetadata[attr].type || `string`;
      let propValue = null;

      if (propType === `string`) {
        propValue = this.getAttribute(attr);
      } else if (propType === `boolean`) {
        propValue = this.isAttributeEnabled(attr);
      } else if (propType === `number`) {
        propValue = this.getNumberAttribute(attr);
      } else if (propType === `json`) {
        propValue = this.this.getJSONAttribute(attr);
      }
