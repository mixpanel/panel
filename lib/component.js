...
class Component extends WebComponent {
  ...
  _updateAttr(attr) {
    ...
    const typeDefaults = {string: ``, boolean: false, number: 0, json: null};
    ...
    if (attrsSchema.hasOwnProperty(attr)) {
      ...
      if (!typeDefaults.hasOwnProperty(attrType)) {
        throw new Error(`Unknown type: ${attrType} for attr: ${attr} in attrsSchema`);
      }

      let attrValue = null;
      if (!this.hasAttribute(attr)) {
        attrValue = attrSchema.hasOwnProperty(`default`) ? attrSchema.default : typeDefaults[attrType];
