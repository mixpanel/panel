...
class Component extends WebComponent {
  ...
  logRenderError(message, error) {
    console.error(message, this, `\n`, error);
  }

  emitRenderError(message, error) {
    this.el.dispatchEvent(new CustomEvent(`rendererror`, {
      detail: {message, error},
      bubbles: true, composed: true,
    }));
  }

  onRenderError(message, error) {
    this.logRenderError(message, error);
    this.emitRenderError(message, error);
  }
  ...
  _render(state) {
    if (this.shouldUpdate(state)) {
      try {
        ...
        this.onRenderError(`Error while rendering ${this.toString()}`, err);
      }
    }
