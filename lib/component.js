...
const PARAM_TYPE_DEFAULTS = new Map([
  [Array, []],
  [String, ``],
  [Boolean, false],
  [Number, 0],
...
class Component extends WebComponent {
  ...
  _syncParams() {
    // maintain local validated map where all schema keys are defined
    this._paramTypes = {};
    const paramTypes = this.constructor.paramTypes || {};

    // vuejs like this.data.$data to reference "real" value of params
    Object.defineProperty(this.params, `$params`, {
      enumerable: false,
      value: {},
    });

    for (const param of Object.keys(paramTypes)) {
      // convert type shorthand to object
      let paramType = paramTypes[param];
      if (PARAM_TYPE_DEFAULTS.has(paramType)) {
        paramType = {type: paramType};
      }

      // Ensure param type is valid
      const type = paramType.type;
      if (!PARAM_TYPE_DEFAULTS.has(type)) {
        const typeString = typeof type === `function` ? type.name : String(type);
        throw new Error(
          `Invalid type: ${typeString} for attr: ${param} in paramType. ` +
            `Only (${Array.from(PARAM_TYPE_DEFAULTS.keys())
              .map((v) => `'${v.name}'`)
              .join(` | `)}) is valid.`,
        );
      }

      if (paramType.default && paramType.required) {
        throw new Error(`${this}: param '${param}' cannot have both required and default`);
      }

      if ([Object, Array, Map, Set].includes(type) && !paramType.shouldUpdate) {
        throw new Error(
          `Object like params '${param}' on ${this.constructor.name} must have 'shouldUpdate' function passed in`,
        );
      }

      const paramSchemaObj = {
        type,
        default: paramType.default ? paramType.default : PARAM_TYPE_DEFAULTS.get(type),
        required: paramType.required ? paramType.required : false,
        // by default use === for comparision
        shouldUpdate: paramType.shouldUpdate || ((oldVal, newVal) => oldVal !== newVal),
      };

      // set default value for the params
      this.params.$params[param] = paramSchemaObj.default;

      this._paramTypes[param] = paramSchemaObj;
    }

    const paramDescription = Object.keys(paramTypes)
      .map((param) => [
        param,
        {
          get() {
    ...
    Object.defineProperties(this.params, paramDescription);
    Object.freeze(this.params);
    return this._paramTypes;
  }

  _enqueueParamUpdate(param, value) {
    this._count = 0;
    if (this._queuedParamUpdates) {
      this._queuedParamUpdates[param] = value;
      return;
    }
    this._queuedParamUpdates = {[param]: value};
    queueMicrotask(() => {
      ...
      this.runHook(`preUpdate`, options, this.state, Object.assign({}, this.params.$params, this._queuedParamUpdates));
      this.updateSelfAndChildren(this._queuedParamUpdates, options);
      this.runHook(`postUpdate`, options, this.state, this.params);
      this._queuedParamUpdates = null;
    });
  }

  _setParams(params) {
    for (const param in params) {
      if (!this._paramTypes[param]) {
        throw new Error(`${param} on ${this.constructor.name} is not defined in schema`);
      }
      const newParam = params[param];
      const oldParam = this.params[param];
      if (this._paramTypes[param].shouldUpdate(oldParam, newParam)) {
        if (this._paramTypes[param].required && (newParam === undefined || newParam === null)) {
          throw new Error(`${param} is required, ${newParam} passed in`);
        }
        this._enqueueParamUpdate(param, newParam);
      }
    }
  }
  ...
  updateSelfAndChildren(stateUpdate, options = {}) {
    ...
    // special handling for `params` container
    if (store === `params`) {
      Object.assign(this.params.$params, stateUpdate);
    } else {
      Object.assign(this[store], stateUpdate);
    }
