...
class Component extends WebComponent {
  ...
  _updateAttr(attr) {
    ...
    if (attrsSchema.hasOwnProperty(attr)) {
      ...
      if (attrSchema.deprecatedMsg) {
        console.warn(`${(this.nodeName || ``).toLowerCase()}: attr '${attr}' is deprecated. ${attrSchema.deprecatedMsg}`);
      }
    }
  }
  ...
  attr(attr) {
    if (attr in this._attrs) {
      ...
      throw new TypeError(`${(this.nodeName || ``).toLowerCase()}: attr '${attr}' is not defined in attrsSchema`);
    }
  }
